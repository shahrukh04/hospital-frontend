...
const Staff = () => {
  ...
  // ✅ Call the function to fetch staff on component mount
  useEffect(() => {
    getAllStaff();
  }, []);
  ...
  const handleSubmit = async (values) => {
    try {
      if (editingStaffId) {
        ...
        // Add new staff
        await api.addStaffMember(values);
  ...
  const fetchAttendance = async (staffId) => {
    console.log("Fetching attendance for staff ID:", staffId); // Add this line
    try {
      const response = await api.getStaffAttendance(staffId);
      setAttendance(response.data);
      setSelectedStaffId(staffId);
      console.log("Attendance data:", response.data); // Add this line
    } catch (error) {
  ...
  // First, let's modify the handleAddAttendance function to check for duplicate dates
const handleAddAttendance = async () => {
  try {
    ...
    if (existingAttendance) {
      ...
      // If confirmed, update the existing record
      await api.updateStaffAttendance(existingAttendance._id, { status });
  ...
  <div className="mt-6 bg-gray-50 p-6 rounded-xl border">
                ...
                <tr key={record._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <button 
                      ...
                      onClick={() => {
                        setAttendanceForm({
                          date: record.date.substring(0, 10), // Format as YYYY-MM-DD for input
                    ...
                    <button 
                      ...
                      onClick={async () => {
                        ...
                        if (result.isConfirmed) {
                          try {
                            await api.deleteStaffAttendance(record._id);
                            toast.success("Attendance deleted!");
                            fetchAttendance(selectedStaffId);
                          } catch (error) {
    ...
    <div className="mt-6 p-4 bg-white rounded-lg shadow">
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        >
          {attendanceForm.date && attendance.some(
            record => new Date(record.date).toDateString() === new Date(attendanceForm.date).toDateString()
          ) ? "Update" : "Add"}
        </button>
      </div>
    </div>
